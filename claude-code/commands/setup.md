...
> The Merge CLI requires `pipx` (and Python 3.10+) to install. Install pipx and re-run `/merge-agent-handler:setup`.
...
This is idempotent. It writes a Merge CLI section to `CLAUDE.md` and updates `.claude/settings.json` so Claude can run `merge` commands. The user may need to restart Claude Code for the new permissions to take effect.
