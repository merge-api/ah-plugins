...
After user selects, you MUST do all three sub-steps (A, B, C) to create a working tool pack:

**Step A — Create the tool pack:**
```bash
curl -s -X POST "https://ah-api.merge.dev/api/v1/tool-packs/" \
  -H "Authorization: Bearer $MERGE_API_KEY" \
  -H "Content-Type: application/json" \
  -H "X-Source: cursor-plugin" \
  -d '{"name": "<user name>'\''s Cursor Tool Pack", "description": "Tool pack created via Cursor Agent", "connectors": [{"connector_id": "<uuid>", "auth_scope": "INDIVIDUAL"}]}'
```
Save the returned `id` as `tool_pack_id`.

**Step B — Discover ALL available tools via tools/list:**
```bash
...
```
Parse the response — each tool has a `name` field (e.g., `gong__list_users`, `jira__create_issue`). Group tool names by their connector slug prefix (the part before `__`).

**Step C — Update the tool pack with ALL discovered tool names:**
```bash
curl -s -X PATCH "https://ah-api.merge.dev/api/v1/tool-packs/<tool_pack_id>/" \
  -H "Authorization: Bearer $MERGE_API_KEY" \
  -H "Content-Type: application/json" \
  -H "X-Source: cursor-plugin" \
  -d '{"connectors": [{"connector_id": "<uuid>", "auth_scope": "INDIVIDUAL", "tool_names": ["connector__tool1", "connector__tool2", ...]}]}'
```
Include the FULL connectors array with ALL tool names for each connector.

**CRITICAL: If you skip Steps B and C, NO tools will be callable. Omitting `tool_names` does NOT auto-include tools — it means ZERO tools.**
